import type { WordEntry, PhaseKey, PhaseProgress } from './types';

function getPhase(word: WordEntry, key: PhaseKey): PhaseProgress {
  return word.learning_phases[key];
}

function progressRatio(phase: PhaseProgress): number {
...
export function isPhaseUnlocked(word: WordEntry, target: PhaseKey): boolean {
