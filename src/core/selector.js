...
export function selectNextCandidate(words, capabilities, history) {
...
function safeParseHistory(raw) {
    ...
    const data = raw;
    const sanitize = (list) => Array.isArray(list) ? list.filter((item) => typeof item === 'string').slice(0, MAX_HISTORY_ITEMS) : [];
    return {
        recentWords: sanitize(data.recentWords),
        recentTestTypes: sanitize(data.recentTestTypes)
    };
}
export function loadSelectorHistory() {
...
export function persistSelectorHistory(history) {
    if (typeof window === 'undefined') {
        return;
    }
    try {
